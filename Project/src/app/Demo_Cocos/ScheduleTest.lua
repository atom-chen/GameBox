...
function ScheduleTest:ctor()
    ...
    参数1： 回调接口用于刷新
    参数2： 每次刷新的时间间隔
    参数3： 是否仅执行一次，false为无限次

    针对于参数3，若设置为 true 的话，也就是 C++中的 scheduleOnce 
    在代码中添加判定的缘故，主要是为了避免重复创建出现问题
    ...
    self._timerScheduler = cc.Director:getInstance():getScheduler():scheduleScriptFunc(timerUpdate, 5, false)
end 
